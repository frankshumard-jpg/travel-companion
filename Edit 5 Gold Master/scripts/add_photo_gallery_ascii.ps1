$pageCards = @{
    ...
    'haugesund.html' = @('Haraldshaugen','Fjord waterfront','Akrafjord','Local seafood','Town harbour')
    'nordfjordeid.html' = @('Fjord reflections','Viking Center','Waterfalls','Mountain views','Nordfjord port')
    'odda.html' = @('Fjord shore','Latefossen','Mountain outlooks','Historic street','Water and glacier views')
    ...
    'alesund.html' = @('Art Nouveau','Harbour view','Mount Aksla','Fjordfront','Alesund streets')
...
foreach ($page in $pageCards.Keys) {
    ...
    $gallery = "      <section class=\"photo-gallery\">`r`n"
    $gallery += "        <h3>Frank & Lynn's Photo Gallery</h3>`r`n"
    $gallery += "        <div class=\"photo-gallery-grid\">`r`n"
